...
int
base_lo_read(int fd, char *buf, int len)
{
	int			status;
...
int
base_lo_write(int fd, const char *buf, int len)
{
	int			status;
	LargeObjectDesc *lobj;
...
be_loread(PG_FUNCTION_ARGS)
{
	...
	totalread = base_lo_read(fd, VARDATA(retval), len);
...
be_lowrite(PG_FUNCTION_ARGS)
{
	...
	totalwritten = base_lo_write(fd, VARDATA_ANY(wbuf), bytestowrite);
